...
DEFAULT_BG = os.path.join(BACKGROUND_FOLDER, "Default.png")


def get_item_info(item_id):
    try:
        r = requests.get(f"https://item-info-neon.vercel.app/info?item_id={item_id}", timeout=5)
...
def get_icon(item_id):
    try:
        r = requests.get(f"https://item-info-neon.vercel.app/icon?item_id={item_id}", timeout=5)
        if r.status_code == 200:
...
def generate_icon(item_id):
    ...
    try:
        ...
        return jsonify({"error": "Background load failed"}), 500

    icon = get_icon(item_id)
    if icon is None:
        return jsonify({"error": "Icon load failed"}), 404

    # Old fit logic
    bg_w, bg_h = background.size
    target_w, target_h = int(bg_w * 0.8), int(bg_h * 0.8)

    icon_w, icon_h = icon.size
    ratio_icon = icon_w / icon_h
    ratio_target = target_w / target_h

    if ratio_icon > ratio_target:
        new_w = target_w
        new_h = int(target_w / ratio_icon)
    else:
        new_h = target_h
        new_w = int(target_h * ratio_icon)
...
# Vercel ke liye ye export zaruri hai
app = app
